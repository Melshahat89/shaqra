...
class GroupController extends AbstractController
{
    ...
    public function store(AddRequestGroup $request){
         return $this->storeOrUpdate($request , null , 'lazyadmin/group');
    }

    public function update($id , UpdateRequestGroup  $request){
        return $this->storeOrUpdate($request , $id , 'lazyadmin/group');
    }
    ...
    public function destroy($id){
        return $this->deleteItem($id , 'lazyadmin/group')->with('sucess' , 'Done Delete group From system');
    }
...
$explode_id = array_map('intval', explode(',', $request->id[0]));
        foreach ($explode_id as $id){
            $this->deleteItem($id);
        } return $this->deleteItem($request->id , 'lazyadmin/group')->with('sucess' , 'Done Delete group From system');
    }
}
