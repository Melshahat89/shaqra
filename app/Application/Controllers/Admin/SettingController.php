...
class SettingController extends AbstractController
{
    ...
    public function destroy($id){
        return $this->deleteItem($id , 'lazyadmin/setting')->with('sucess' , 'Done Delete setting From system');
    }
...
$explode_id = array_map('intval', explode(',', $request->id[0]));
        foreach ($explode_id as $id){
            $this->deleteItem($id);
        } return $this->deleteItem($request->id , 'lazyadmin/setting')->with('sucess' , 'Done Delete setting From system');
    }
}
