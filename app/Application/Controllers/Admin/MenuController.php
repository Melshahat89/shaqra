...
class MenuController extends AbstractController
{
    ...
    public function store(AddRequestMenu $request){
         return $this->storeOrUpdate($request , null , 'lazyadmin/menu');
    }


    public function update($id , UpdateRequestMenu $request){
        return $this->storeOrUpdate($request , $id , 'lazyadmin/menu');
    }
    ...
    public function destroy($id){
        return $this->deleteItem($id , 'lazyadmin/menu')->with('sucess' , 'Done Delete menu From system');
    }
...
$explode_id = array_map('intval', explode(',', $request->id[0]));
        foreach ($explode_id as $id){
            $this->deleteItem($id);
        } return $this->deleteItem($request->id , 'lazyadmin/menu')->with('sucess' , 'Done Delete menu From system');
    }
