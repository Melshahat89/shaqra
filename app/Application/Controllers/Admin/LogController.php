...
class LogController extends AbstractController
{
    ...
    public function destroy($id){
        return $this->deleteItem($id , 'lazyadmin/log')->with('sucess' , 'Done Delete log From system');
    }
...
$explode_id = array_map('intval', explode(',', $request->id[0]));
        foreach ($explode_id as $id){
            $this->deleteItem($id);
        } return $this->deleteItem($request->id , 'lazyadmin/log')->with('sucess' , 'Done Delete log From system');
    }
}
