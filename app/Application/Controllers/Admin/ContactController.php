...
class ContactController extends AbstractController
{
    ...
    public function destroy($id){
        return $this->deleteItem($id , 'lazyadmin/contact')->with('sucess' , 'Done Delete contact From system');
    }
...
$explode_id = array_map('intval', explode(',', $request->id[0]));
        foreach ($explode_id as $id){
            $this->deleteItem($id);
        } return $this->deleteItem($request->id , 'lazyadmin/contact')->with('sucess' , 'Done Delete contact From system');
    }
