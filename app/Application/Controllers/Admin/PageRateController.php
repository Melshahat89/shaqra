...
use \App\Application\Requests\Admin\PageRate\AddRequestPageRate;
use \App\Application\Requests\Admin\PageRate\UpdateRequestPageRate;
   ...
   public function __construct(PageRate $model , Page $parent)
    ...
    public function addRate($id , AddRequestPageRate $request){
        if($this->checkIfUserRateBefore($id)){
            ...
            return redirect('lazyadmin/page/'.$id.'/view');
        }
        $array = [
            'rate' => $request->rate,
