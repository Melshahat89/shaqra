...
trait MainProcessTrait {
    ...
    public function storeOrUpdateQuestionChoices($request, $id){

        foreach($request->all() as $key => $val) {

            if(str_contains($key, 'choice')) {
                
                if(array_key_exists('id', $val)){

                    $answer = Quizquestionschoice::find($val['id']);
                }else{
                    $answer = new Quizquestionschoice();
                }


                $answer->choice = json_encode($val['choice'], JSON_UNESCAPED_UNICODE);
                $answer->is_correct = (isset($val['is_correct'])) ? $val['is_correct'] : 0;
                $answer->quizquestions_id = $id;
                $answer->save();
    ...
    public function deepReplicate($model){
        $old = $model;
        $old->load('courserelated');
        $copy = $old->replicate();
        $copy->slug = "random-slug" . rand(1,1000);
        $copy->save();

        // foreach($old->courserelated as $item){
        //     $newOption = $item->replicate();
        //     $newOption->courses_id = $copy->id;
        //     $newOption->save();
        // }

        foreach($old->courseincludes as $item){
            $newOption = $item->replicate();
            $newOption->courses_id = $copy->id;
            $newOption->save();
        }
