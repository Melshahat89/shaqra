...
trait UploadTrait{
        ...
        if($request->file($field) != null){
            ...
            if(is_array($request->file($field))){
                foreach($request->file($field)  as $file){
                    ...
                    }elseif(getFileType($field , $file->getClientOriginalName()) == 'File'){
                ...
                }elseif(getFileType($field , $request->file($field)->getClientOriginalName()) == 'File'){
    ...
    protected function uploadFileOrMultiUpload($image , $destinationPath,$field){
        ...
        //MEDIUM IMAGE
        $image->resize(path(env('SMALL_IAMGE_WIDTH2')), path(env('SMALL_IAMGE_HEIGHT2')) , function ($constraint) {
            $constraint->aspectRatio();
            $constraint->upsize();
        });

        $image->save(path(env('MEDIUM_IMAGE_PATH').DS.$fileName) , path(env('IMAGE_RESLUTION2')));
