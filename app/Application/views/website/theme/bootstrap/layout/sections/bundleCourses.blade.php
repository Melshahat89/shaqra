...
@if(userCountry()['code'] == "EG")
    @if($data->discount_egp > 0)
		...
		<figure class="img item promoted-course" style="--colorcode: {{isset($data->categories->color_code) ? $data->categories->color_code : '#E6E6E6'}}" data-awards="{{$data->discount_egp}}% {{trans('home.discount')}}">
	@else
		<figure class="img">
    @endif
@else
    @if($data->discount_usd > 0)
        @php $discountApplied = 1; @endphp
	    <figure class="img item promoted-course" style="--colorcode: {{isset($data->categories->color_code) ? $data->categories->color_code : '#E6E6E6'}}" data-awards="{{$data->discount_usd}}% {{trans('home.discount')}}">
    @else
	    <figure class="img">s
    @endif
@endif
