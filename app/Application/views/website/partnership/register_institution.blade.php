        ...
        <div class="contianer">
            ...
            <div class="partnership_form">
                <form class="login-form form-vertical" role="form" method="POST"
                    action="{{ concatenateLangToUrl('partnership/register-institution') }}">
                    {{ csrf_field() }}
                    <div class="row">
                        <div class="col-md-6">
                        ...
                        <div class="col-md-12">
                            <div class="input-group mb-20">
                                <textarea class="form-control input-item" name="about" required rows="6"
                                    placeholder="About your institute"></textarea>

                                    {{--  {!! extractFiled(isset($item) ? $item : null , "about", isset($item->about) ? $item->about : old("about") , "textarea", "user", 'form-control input-item' ) !!}  --}}
                        ...
                        <div class="col-md-12 d-none">
                            <div class="input-group mb-20">
                                {{--  { extractFiled(isset($item) ? $item : null, 'instructorname', old('instructorname'), 'text',
                                'user', 'form-control input-item') !!}!!  --}}

                                <input type="text" name="instructorname" class="form-control input-item" placeholder="{{ trans('user.instructorname') }}">



                                
                            </div>
                            @if ($errors->has('instructorname.en'))
                                    <div class="alert alert-danger">
                                        <span class='help-block'>
                                            <strong>{{ $errors->first('instructorname.en') }}</strong>
                                        </span>
                                    </div>
                                @endif
                                @if ($errors->has('instructorname.ar'))
                                    <div class="alert alert-danger">
                                        <span class='help-block'>
                                            <strong>{{ $errors->first('instructorname.ar') }}</strong>
                                        </span>
                                    </div>
                                @endif
                        </div>
                        ...
                        <div class="col-md-10 mt-10 ">
                            <form method="post" id="image-form">
                              ...
                              <div class="input-group">
                                <div id="msg">{{ trans('partnership.Please upload the institution logo') }}</div>
                                <button type="button" class="browse btn btn-primary">{{ trans('account.Upload image') }}</button>
                              </div>
                            </form>
                        </div>
                
                        <div class="col-md-2 text-right">
                          <img id="preview" src="{{ asset('partnership') }}/images/new/Preview-icon.png" alt="...">
                        </div>
