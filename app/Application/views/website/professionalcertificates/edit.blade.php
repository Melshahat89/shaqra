...
<div class="pull-{{ getDirection() }} col-lg-9">
        ...
        <form action="{{ concatenateLangToUrl('professionalcertificates/item') }}{{ isset($item) ? '/'.$item->id : '' }}" method="post" enctype="multipart/form-data">
            {{ csrf_field() }}
            @include("website.professionalcertificates.relation.courses.edit")
                <div class="form-group {{ $errors->has("startdate") ? "has-error" : "" }}" > 
   <label for="startdate">{{ trans("professionalcertificates.startdate")}}</label>
    <input type="date" name="startdate" class="form-control datepicker" id="startdate" value="{{ isset($item->startdate) ? $item->startdate : old("startdate") }}"  placeholder="{{ trans("professionalcertificates.startdate")}}">
  </div>
   ...
   <label for="registrationdeadline">{{ trans("professionalcertificates.registrationdeadline")}}</label>
    <input type="text" name="registrationdeadline" class="form-control" id="registrationdeadline" value="{{ isset($item->registrationdeadline) ? $item->registrationdeadline : old("registrationdeadline") }}"  placeholder="{{ trans("professionalcertificates.registrationdeadline")}}">
  </div>
   ...
   @endif
             <div class="form-group">
                <button type="submit" name="submit" class="btn btn-default" >
                    ...
                    {{ trans('website.Update') }}  {{ trans('website.professionalcertificates') }}
                </button>
            </div>
        </form>
</div>
