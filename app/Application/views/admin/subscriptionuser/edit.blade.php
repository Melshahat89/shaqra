...
@endsection
 @section('content')
    @component(layoutForm() , ['title' => trans('subscriptionuser.subscriptionuser') , 'model' => 'subscriptionuser' , 'action' => isset($item) ? trans('home.edit')  : trans('home.add')  ])
         @include(layoutMessage())
        <form action="{{ concatenateLangToUrl('lazyadmin/subscriptionuser/item') }}{{ isset($item) ? '/'.$item->id : '' }}" method="post" enctype="multipart/form-data">
            {{ csrf_field() }}
            @include("admin.subscriptionuser.relation.orders.edit")
            @include("admin.subscriptionuser.relation.user.edit")
   ...
   <label for="start_date">{{ trans("subscriptionuser.start_date")}}</label>
    <input type="date" name="start_date" class="form-control " id="start_date" value="{{ isset($item->start_date) ? $item->start_date : old("start_date") }}"  placeholder="{{ trans("subscriptionuser.start_date")}}">
  </div>
   ...
   <label for="end_date">{{ trans("subscriptionuser.end_date")}}</label>
    <input type="date" name="end_date" class="form-control " id="end_date" value="{{ isset($item->end_date) ? $item->end_date : old("end_date") }}"  placeholder="{{ trans("subscriptionuser.end_date")}}">
  </div>
   ...
   @endif
              <div class="form-group">
                <button type="submit" name="submit" class="btn btn-default" >
                    ...
                    {{ trans('home.save') }}  {{ trans('subscriptionuser.subscriptionuser') }}
                </button>
            </div>
        </form>
    @endcomponent
