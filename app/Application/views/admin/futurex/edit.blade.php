...
@endsection
 @section('content')
    @component(layoutForm() , ['title' => trans('futurex.futurex') , 'model' => 'futurex' , 'action' => isset($item) ? trans('home.edit')  : trans('home.add')  ])
        ...
        <form action="{{ concatenateLangToUrl('admin/futurex/item') }}{{ isset($item) ? '/'.$item->id : '' }}" method="post" enctype="multipart/form-data">
            {{ csrf_field() }}
            @include("admin.futurex.relation.user.edit")
