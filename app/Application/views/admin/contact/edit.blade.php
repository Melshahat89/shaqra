...
@section('content')
    @component(layoutForm() , ['title' => trans('contact.contact') , 'model' => 'contact' , 'action' => isset($item) ? trans('home.edit')  : trans('home.add'),  'button' => false  ])
        ...
        <form class="card-body" action="{{ concatenateLangToUrl('lazyadmin/contact/item') }}{{ isset($item) ? '/'.$item->id : '' }}" method="post" enctype="multipart/form-data">
            {{ csrf_field() }}
            ...
            <div class="form-group">
                <label class="col-md-2 col-form-label" for="email">{{ trans('contact.email') }}</label>
                <input type="email" name="email" id="email" value="{{ isset($item) ? $item->email : old('email') }}" class="form-control"/>
            </div>


            <div class="form-group">
                <label class="col-md-2 col-form-label" for="subject">{{ trans('contact.subject') }}</label>
                <input type="text" name="subject" id="subject" value="{{ isset($item) ? $item->subject : old('subject') }}" class="form-control"/>
            </div>


            <div class="form-group">
                <label class="col-md-2 col-form-label" for="phone">{{ trans('contact.phone') }}</label>
                <input type="text" name="phone" id="phone" value="{{ isset($item) ? $item->phone : old('phone') }}" class="form-control"/>
            </div>


            <div class="form-group">
                <label class="col-md-2 col-form-label" for="experts">{{ trans('contact.message') }}</label>
