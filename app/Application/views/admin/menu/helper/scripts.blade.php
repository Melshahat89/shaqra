...
<script>
    var url = "{{ concatenateLangToUrl('lazyadmin/update/menuItem')  }}";
    var id = "{{ $item->id }}";
    ...
    $('#defaultModal').on('shown.bs.modal', function(evt) {
        ...
        if($(evt.relatedTarget).data('url') && $(evt.relatedTarget).data('id')){
            ...
            $.get("{{ concatenateLangToUrl('lazyadmin/getItemInfo/') }}/"+id , function(result){
    ...
    function UpdateItem(){
        ...
        $.post("{{ concatenateLangToUrl('lazyadmin/updateOneMenuItem/') }}/",data, function(result){
