...
@endsection
 @section('content')
    @component(layoutForm() , ['title' => trans('page.page') , 'model' => 'page' , 'action' => isset($item) ? trans('home.edit')  : trans('home.add')  ])
        ...
        <form class="card-body" action="{{ concatenateLangToUrl('lazyadmin/page/item') }}{{ isset($item) ? '/'.$item->id : '' }}"
              method="post" enctype="multipart/form-data">
            {{ csrf_field() }}
             <div class="form-group {{  $errors->has("title.en")  &&  $errors->has("title.ar")  ? "has-error" : "" }}">
                <label class="col-md-2 col-form-label" for="title">{{ trans("page.title")}}</label>
            ...
            <div class="form-group {{  $errors->has("body.en")  &&  $errors->has("body.ar")  ? "has-error" : "" }}">
                <label class="col-md-2 col-form-label" for="body">{{ trans("page.body")}}</label>
            ...
            <div class="form-group {{  $errors->has("active")   ? "has-error" : "" }}">
                <label class="col-md-2 col-form-label" for="active">{{ trans("page.active")}}</label>
