		...
		@foreach($comments as $comment)
			<li>
				<div>
					...
					<p>{{ $comment->comment}}</p>
@if(auth()->check() && $comment->user_id == auth()->user()->id)			<a href="{{concatenateLangToUrl("admin/page/delete/comment/".$comment->id)}}" class="btn btn-danger"><i class="fa fa-trash"></i></a>
			<span class="btn btn-info" onclick="$(this).next().slideToggle()"><i class="fa fa-edit"></i></span>
	<form method="post" action="{{ concatenateLangToUrl("admin/page/update/comment/".$comment->id) }}" style="display:none">	{{ csrf_field() }}
