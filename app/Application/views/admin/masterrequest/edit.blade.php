...
@endsection
 @section('content')
    @component(layoutForm() , ['title' => trans('masterrequest.masterrequest') , 'model' => 'masterrequest' , 'action' => isset($item) ? trans('home.edit')  : trans('home.add')  ])
         @include(layoutMessage())
        <form class="card-body" action="{{ concatenateLangToUrl('lazyadmin/masterrequest/item') }}{{ isset($item) ? '/'.$item->id : '' }}" method="post" enctype="multipart/form-data">
            {{ csrf_field() }}
            @include("admin.masterrequest.relation.courses.edit")
            @include("admin.masterrequest.relation.user.edit")
     <div class="form-group {{ $errors->has("qualification") ? "has-error" : "" }}" > 
   <label class="col-md-2 col-form-label" for="qualification">{{ trans("masterrequest.qualification")}}</label>
   ...
   <label class="col-md-2 col-form-label" for="collage_name">{{ trans("masterrequest.collage_name")}}</label>
   ...
   <label class="col-md-2 col-form-label" for="section">{{ trans("masterrequest.section")}}</label>
   ...
   <label class="col-md-2 col-form-label" for="g_year">{{ trans("masterrequest.g_year")}}</label>
   ...
   <label class="col-md-2 col-form-label" for="address">{{ trans("masterrequest.address")}}</label>
   ...
   <label class="col-md-2 col-form-label" for="documentation">{{ trans("masterrequest.documentation")}}</label>
   ...
   @endif
    ...
    <label class="col-md-2 col-form-label" for="status">{{ trans("masterrequest.status")}}</label>
    ...
    @endif
              <div class="form-group">
                <button type="submit" name="submit" class="btn btn-info w-md" >
                    ...
                    {{ trans('home.save') }}  {{ trans('masterrequest.masterrequest') }}
                </button>
            </div>
        </form>
    @endcomponent
