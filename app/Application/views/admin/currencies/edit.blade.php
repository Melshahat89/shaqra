...
@section('content')
    @component(layoutForm() , ['title' => trans('currencies.currencies') , 'model' => 'currencies' , 'action' => isset($item) ? trans('home.edit')  : trans('home.add')  ])
         @include(layoutMessage())
        <form action="{{ concatenateLangToUrl('lazyadmin/currencies/item') }}{{ isset($item) ? '/'.$item->id : '' }}" method="post" enctype="multipart/form-data">
            {{ csrf_field() }}
		 ...
		 <div class="form-group {{ $errors->has("b2c_yearly_discount_perc") ? "has-error" : "" }}" > 
            ...
            <div class="form-group">
                <button type="submit" name="submit" class="btn btn-default" >
                    ...
                    {{ trans('home.save') }}  {{ trans('currencies.currencies') }}
                </button>
            </div>
        </form>
    @endcomponent
