...
@section('content')
    @component(layoutForm() , ['title' => trans('social.social') , 'model' => 'social' , 'action' => isset($item) ? trans('home.edit')  : trans('home.add')  ])
         @include(layoutMessage())
        <form class="card-body" action="{{ concatenateLangToUrl('lazyadmin/social/item') }}{{ isset($item) ? '/'.$item->id : '' }}" method="post" enctype="multipart/form-data">
            {{ csrf_field() }}

 		 <div class="form-group {{ $errors->has("user_id") ? "has-error" : "" }}" > 
			<label class="col-md-2 col-form-label" for="user_id">{{ trans("social.user_id")}}</label>
		 ...
		 <div class="form-group {{ $errors->has("provider") ? "has-error" : "" }}" > 
			<label class="col-md-2 col-form-label" for="provider">{{ trans("social.provider")}}</label>
		 ...
		 <div class="form-group {{ $errors->has("identifier") ? "has-error" : "" }}" > 
			<label class="col-md-2 col-form-label" for="identifier">{{ trans("social.identifier")}}</label>
		 ...
		 <div class="form-group {{ $errors->has("profile_cache") ? "has-error" : "" }}" > 
			<label class="col-md-2 col-form-label" for="profile_cache">{{ trans("social.profile_cache")}}</label>
		 ...
		 <div class="form-group {{ $errors->has("session_data") ? "has-error" : "" }}" > 
			<label class="col-md-2 col-form-label" for="session_data">{{ trans("social.session_data")}}</label>
            ...
            <div class="form-group">
                <button type="submit" name="submit" class="btn btn-info w-md" >
                    ...
                    {{ trans('home.save') }}  {{ trans('social.social') }}
                </button>
            </div>
        </form>
    @endcomponent
