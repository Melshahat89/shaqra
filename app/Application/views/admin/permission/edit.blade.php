...
@section('content')
    ...
    <form class="card-body" action="{{ concatenateLangToUrl('lazyadmin/permission/item') }}{{ isset($item) ? '/'.$item->id : '' }}" method="post" enctype="multipart/form-data">
        {{ csrf_field() }}
        ...
        <div class="form-group">
            <div class="form-line">
                <label class="col-md-2 col-form-label" for="">{{ trans('admin.Not Allow')}}
                    <input type="radio" name="permission" {{ isset($item) && $item->permission == 0 ? 'checked' : '' }} value="0">
                </label>
                <label class="col-md-2 col-form-label" for="">{{ trans('admin.Allow')}}
