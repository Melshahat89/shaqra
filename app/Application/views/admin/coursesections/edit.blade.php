...
@endsection
 @section('content')
    @component(layoutForm() , ['title' => trans('coursesections.coursesections') , 'model' => 'coursesections' , 'action' => isset($item) ? trans('home.edit')  : trans('home.add')  ])
         @include(layoutMessage())
        <form class="card-body" action="{{ concatenateLangToUrl('lazyadmin/coursesections/item') }}{{ isset($item) ? '/'.$item->id : '' }}" method="post" enctype="multipart/form-data">
            {{ csrf_field() }}
            @include("admin.coursesections.relation.courses.edit")
     <div class="form-group  {{  $errors->has("title.en")  &&  $errors->has("title.ar")  ? "has-error" : "" }}" >
   <label class="col-md-2 col-form-label" for="title">{{ trans("coursesections.title")}}</label>
   ...
   <label class="col-md-2 col-form-label" for="will_do_at_the_end">{{ trans("coursesections.will_do_at_the_end")}}</label>
   ...
   <label class="col-md-2 col-form-label" for="position">{{ trans("coursesections.position")}}</label>
   ...
   @endif
              <div class="form-group">
                <button type="submit" name="submit" class="btn btn-info w-md" >
                    ...
                    {{ trans('home.save') }}  {{ trans('coursesections.coursesections') }}
                </button>
            </div>
        </form>
    @endcomponent
