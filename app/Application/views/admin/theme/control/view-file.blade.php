...
@section('content')
    ...
    <div class="col-lg-10">
        <form class="card-body" action="{{ concatenateLangToUrl('lazyadmin/theme/save-file?file='.$fileName) }}" method="Post">
            {{ csrf_field() }}
    ...
    <div class="col-lg-2">
        ...
        <ol>
            @foreach($files as $file)
                <li><a href="{{ url('lazyadmin/theme/open-file?file='.$file.'&type='.$theme) }}">{{ $file }}</a></li>
            @endforeach
        </ol>
    </div>
@endsection
