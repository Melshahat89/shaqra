...
@section('content')
    @component(layoutForm() , ['title' => trans('achievements.achievements') , 'model' => 'achievements' , 'action' => isset($item) ? trans('home.edit')  : trans('home.add')  ])
         @include(layoutMessage())
        <form action="{{ concatenateLangToUrl('lazyadmin/achievements/item') }}{{ isset($item) ? '/'.$item->id : '' }}" method="post" enctype="multipart/form-data">
            {{ csrf_field() }}
		 ...
		 <div class="form-group {{ $errors->has("logo") ? "has-error" : "" }}" > 
            ...
            <div class="form-group">
                <button type="submit" name="submit" class="btn btn-default" >
                    ...
                    {{ trans('home.save') }}  {{ trans('achievements.achievements') }}
                </button>
            </div>
        </form>
    @endcomponent
